            ...
            <div class="card-body">
                <form action="{{route('update.service',$services->id)}}" method="post">
                    ...
                    <div class="form-group">
                        ...
                        <input type="text" name="title" value="{{$services->title}}" class="form-control" id="exampleFormControlInput1" placeholder="About Title">
                        @error('title')<span class="text-danger">{{$message}}</span>@enderror
                    </div>

                    <div class="form-group">
                        <label for="exampleFormControlInput1">Icon class</label>
                        <input type="text" name="icon" value="{{$services->icon}}" class="form-control" id="exampleFormControlInput1" placeholder="About Title">
                        @error('icon')<span class="text-danger">{{$message}}</span>@enderror
                    </div>

                    <div class="form-group">
                        <label for="exampleFormControlTextarea1">Service Description</label>
                        <textarea class="form-control" name="des" id="exampleFormControlTextarea1" rows="3">{{$services->des}}</textarea>
